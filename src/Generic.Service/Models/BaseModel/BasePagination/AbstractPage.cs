...
namespace Generic.Service.Models.BaseModel.BasePagination {
    public abstract class AbstractPage<TValue, TResult> : IBasePage<TResult>
        where TValue : class
    where TResult : class {
        ...
        protected readonly IBaseConfigurePage _config;
        protected readonly IQueryable<TValue> _listEntities;
        protected readonly int _count;
        #endregion

        #region Ctor
        public AbstractPage (IQueryable<TValue> listEntities, Func<IEnumerable<TValue>, IEnumerable<TResult>> mapperTo, IBaseConfigurePage config, bool pageStartInOne, string defaultSort, string defaultOrder, int defaultSize) {
            if (mapperTo == null)
        ...
        protected virtual void ValidateCtor (int count, IQueryable<TValue> listEntities, IBaseConfigurePage config) {
    ...
    public abstract class AbstractPage<TValue> : AbstractPage<TValue, TValue>
        where TValue : class {
            #region Ctor
            public AbstractPage (IQueryable<TValue> listEntities, IBaseConfigurePage config, bool pageStartInOne, string defaultSort, string defaultOrder, int defaultSize) : base (listEntities, null, config, pageStartInOne, defaultSort, defaultOrder, defaultSize) { }
            #endregion
